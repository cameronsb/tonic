...
export function ConfigBar() {
  ...
  const toggleDrawer = useCallback(() => {
    setIsDrawerOpen((prev) => {
      // If opening the drawer and onboarding is active, skip the onboarding
      if (!prev && !settings.onboarding.completed) {
        skipOnboarding();
      }
      return !prev;
    });
  }, [settings.onboarding.completed, skipOnboarding]);
      ...
      <header className="header-bar">
        <div className="header-brand">
          ...
          <select
            ...
            onChange={(e) => {
              const [key, mode] = e.target.value.split('-') as [
                (typeof NOTES)[number],
                'major' | 'minor',
              ];
      ...
      <aside className={`settings-drawer ${isDrawerOpen ? 'open' : ''}`}>
        ...
        <div className="drawer-content">
          ...
          <section className="settings-section">
            ...
            <div className="setting-item">
              <label className="setting-label" htmlFor="key-select">
                Key
              </label>
              <select
                id="key-select"
            ...
            <div className="setting-item">
              <label className="setting-label" htmlFor="scale-select">
                Scale
              </label>
              <select
                id="scale-select"
